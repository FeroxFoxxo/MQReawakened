...
public static class LoadRoomData
{
    ...
    public static Dictionary<int, List<BaseSyncedEntity>> LoadEntities(this Room room, ReflectionUtils reflectionUtils, IServiceProvider services, out Dictionary<int, List<string>> unknownEntities)
    {
            ...
            foreach (var entity in plane.Value.GameObjects)
                foreach (var component in entity.Value.ObjectInfo.Components)
                {
                    ...
                    if (syncedEntities.TryGetValue(mqType.FullName!, out var internalType))
                    {
                        ...
                        foreach (var componentValue in component.Value.ComponentAttributes.Where(componentValue => !string.IsNullOrEmpty(componentValue.Value)))
                        {
                            ...
                            if (field.FieldType == typeof(string))
                                ...
                                room.Logger.LogError("It is unknown how to convert a string to a {FieldType}.", field.FieldType);
                        }

                        var storedData = new StoredEntityModel(entity.Value, room);

                        var instancedEntity = reflectionUtils.CreateBuilder<BaseSyncedEntity>(internalType.GetTypeInfo()).Invoke(services);
