...
public class EnemyBomber(Room room, string entityId, BaseComponent baseEntity, IServiceProvider services) : Enemy(room, entityId, baseEntity, services)
{
    ...
    public override void HandleAggro()
    {
        ...
        {
            Room.SendSyncEvent(Utils.AISyncEventHelper.AIDo(Entity, Position, 1.0f, BehaviorList.IndexOf("Bomber"), string.Empty, AiData.Sync_TargetPosX, AiData.Sync_TargetPosY,
            AiData.Intern_Dir, false));

            AiBehavior = ChangeBehavior("Bomber");
            _behaviorEndTime = ResetBehaviorTime(Convert.ToSingle(BehaviorList.GetBehaviorStat("Bomber", "inTime")) + Convert.ToSingle(BehaviorList.GetBehaviorStat("Bomber", "loopTime")));
        }
    }
