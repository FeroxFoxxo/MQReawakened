...
public class Bomber(BehaviorEnemy enemy) : IBomber
{
    public override void bomb(int direction)
    {
        if (enemy.CurrentBehavior is AIBehaviorStinger stinger)
        {
            ...
            enemy.Room.ExplodeBomb(null, enemy.Position, stingerProperties.damageDistance, enemy.GetDamage(), Elemental.Standard, enemy.TimerThread);
        }
        else if (enemy.CurrentBehavior is AIBehaviorStomper stomper)
        {
            ...
            enemy.Room.ExplodeBomb(null, new Vector3(enemy.Position.x + (direction > 0 ? 1 : -1) * stomperProperties.damageOffset, enemy.Position.y, enemy.Position.z), stomperProperties.damageDistance, enemy.GetDamage(), Elemental.Standard, enemy.TimerThread);
        }
        else if (enemy.CurrentBehavior is AIBehaviorBomber bomber)
        {
            var bomberProperties = bomber.GetProperties() as BomberProperties;
            enemy.Room.ExplodeBomb(null, enemy.Position, bomberProperties.bombRadius, enemy.GetDamage(), Elemental.Standard, enemy.TimerThread);
        }
        else
