...
public class Bomber(BehaviorEnemy enemy) : IBomber
{
    public override void bomb(int direction)
    {
        if (enemy.AiBehavior is AIBehaviorStinger stinger)
            ...
            enemy.Room.ExplodeBomb(null, new Vector3(enemy.Position.x + (direction > 0 ? 1 : -1) * stomper.DamageOffset, enemy.Position.y, enemy.Position.z), stomper.DamageDistance, enemy.GetDamage(), Elemental.Standard, enemy.TimerThread);
