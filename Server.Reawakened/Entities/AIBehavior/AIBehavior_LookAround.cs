...
{
    private readonly float LookAround_InitialProgressRatio = LookAround_InitialProgressRatio;
    private readonly bool LookAround_SnapOnGround = LookAround_SnapOnGround;
    ...
    public override float GetBehaviorRatio(AIProcessData aiData, float clockTime)
    {
        ...
        {
            var num = (int)((clockTime - _behaviorStartTime) / (_behaviorDuration / aiData.Sync_SpeedFactor)) + 1;
        ...
        var num2 = (clockTime - _behaviorStartTime) / (_behaviorDuration / aiData.Sync_SpeedFactor);
